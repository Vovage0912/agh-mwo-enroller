...
public class ParticipantRestController {
	...
	public ResponseEntity<?> removeParticipant(@PathVariable("id") String login) {
		...
		if (foundParticipant == null) {
			return new ResponseEntity("User doesen't exist", HttpStatus.NOT_FOUND);
		}

		participantService.remove(foundParticipant);
		return new ResponseEntity<Participant>(foundParticipant, HttpStatus.OK);
	}

	@RequestMapping(value = "/{id}", method = RequestMethod.PUT)
	public ResponseEntity<?> updateParticipant(@PathVariable("id") String login, @RequestBody String password) {
		Participant foundParticipant = participantService.findByLogin(login);
		if (foundParticipant == null) {
			return new ResponseEntity("User doesen't exist", HttpStatus.NOT_FOUND);
		}
